...
const DEFAULT_HOST = "http://https://medicare-p53k.vercel.app".replace(/\/$/, "");
...
export default function ServiceDetail() {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ FIX 2: Frontend validation — Online payment minimum ₹50 check
    if (paymentMethod === "Online" && (service?.price ?? 0) < 50) {
      setSubmitError(
        "Minimum online payment amount is ₹50. Please use Cash payment for smaller amounts.",
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return; // finally block will call setSubmitting(false)
      }

      const { appointment, checkoutUrl } = json || {};

      if (checkoutUrl) {
        // ✅ FIX 1: setSubmitting(false) before redirect so button doesn't stay stuck
        setSubmitting(false);
      ...
      setSubmitting(false); // ✅ সবসময় চলবে — error হলেও বাটন unlock হবে
    }
  };
