...
const ListPage = () => {
  ...
  async function fetchAppointments() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let msg = `Failed to fetch appointments (${res.status})`;
  ...
  async function updateStatusRemote(id, newStatus) {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        let msg = `Status update failed (${res.status})`;
        try { msg = JSON.parse(text)?.message || msg; } catch {}
        throw new Error(msg);
      }
      const body = await res.json();
  ...
  async function rescheduleRemote(id, newDate, newTime24) {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        let msg = `Reschedule failed (${res.status})`;
        try { msg = JSON.parse(text)?.message || msg; } catch {}
        throw new Error(msg);
      }
      const body = await res.json();
          ...
          <div className={listPageStyles.searchFilterContainer}>
            <div className={listPageStyles.searchContainer}>
              <div className={listPageStyles.searchIconContainer}>
                <SearchIcon className={listPageStyles.searchIcon} />
              </div>
              <input
                value={search}
          ...
          <div className={listPageStyles.appointmentsGrid}>
            {filtered.map((a) => (
              <article key={a.id} className={listPageStyles.appointmentCard}>
                ...
                <div className={listPageStyles.rescheduleSection}>
                  <RescheduleButton
                    appointment={a}
                    onReschedule={(d, t) =>
                      updateDateTime(a.id, d, t)
                    }
                  />
                </div>
              </article>
