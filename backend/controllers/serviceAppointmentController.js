...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    if (!clerkUserId) return res.status(401).json({
      success: false,
      message: "Authentication required to creat a service appointment"
    });
    ...
    if (numericAmount < minAmount) {
      return res.status(400).json({
        success: false,
        message: `Minimum online payment amount is ₹${minAmount}. Please use Cash payment for smaller amounts.`
      });
    }
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (!stripe) {
      return res.status(500).json({ success: false, message: "Stripe is not setup" });
    }
    ...
    if (session.payment_status !== "paid") return res.status(400).json({ success: false, message: "Payment not confirmed" });
    ...
    return res.json({ success: true, appoinment: appt });
  } catch (err) {
...
export const getServiceAppointmentsStats = async (req, res) => {
  try {
    ...
    console.error("getServiceAppointmentsStats error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
...
export const getServiceAppointmentsBypatient = async (req, res) => {
  try {
    ...
    if (mobile) filter.mobile = mobile;
    const appointments = await ServiceAppointment.find(filter).sort({ createdAt: -1 }).lean();
