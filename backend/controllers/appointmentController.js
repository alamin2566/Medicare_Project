...
export const getAppointments = async (req, res) => {
  try {
    ...
    const items = await Appointment.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit)
      .populate("doctorId", "name speciality owner imageUrl").lean();
...
export const getAppointmentsByPatient = async (req, res) => {
  try {
    ...
    console.log("Resolved createdBy for patient appointments:", resolvedCreatedBy);
...
export const createAppointment = async (req, res) => {
  try {
    ...
    if (!clerkUserId) {
      return res.status(401).json({
        success: false,
        message: "Authentication required to create appointment"
      });
    }
    ...
    if (existingBooking) {
      return res.status(409).json({
        success: false,
        message: "You already have a appoinment with this doctor at the selected slot."
      });
    }
    ...
    try {
      ...
      console.warn("Error fetching doctor details:", e?.message || e);
    }
    if (!doctor) {
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    const updated = await Appointment.findByIdAndUpdate(id, update, { new: true,
      runValidators: true }).populate({ path: "doctorId", select: "name imageUrl" }).lean();
...
export const cancelAppoinment = async (req, res) => {
  try {
    ...
    console.error("cancelAppoinment error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
