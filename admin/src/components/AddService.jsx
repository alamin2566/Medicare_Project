...
import { CheckCircle, XCircle, AlertTriangle, Clock, ImageIcon, Plus, Trash2, Calendar } from 'lucide-react';
...
const AddService = ({serviceId}) => {
  ...
  function addSlot() {
    ...
    if (slots.includes(formatted)) {
      showToast("error", "Duplicate Slot", "This time slot has already been added. Please select a different time.");
      return;
    }

    if (isSelectedDateTimeInPast()) {
      showToast("error", "Past Time", "You cannot add a time slot in the past. Please select a future date/time.");
      ...
      <div className={addServiceStyles.toast.container}>
        {toast && (
          ...
          >
            ...
            >
              <XCircle className="w-4 h-4 text-gray hover:text-gray-600"/>
            </button>
          </div>
        )}
      </div>
        ...
        justify-between mb-6 sm:mb-8 gap-4">
          ...
          <div className={addServiceStyles.headerActions}>
            ...
            <button type="submit" disabled={submitting} className={addServiceStyles.buttons.submit}>
              {submitting ? (
                <>
                  <div className="w-4 h-4 border-white border-t-transparent
